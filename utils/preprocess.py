...
def skull_strip(options):
    ...
    if options['reg_space'] == 'FlairtoT1':
        ...
        try:
            print("> PRE:", scan, "skull_stripping the " + CURL + "T1" + CEND +  " modality")
            if os_host == 'Windows':
                print("skull_stripping the " + CURL + "T1" + CEND +  " modality [",os_host, " system]")
                ...
                print("> ERROR:", scan, "registering masks(T1), quiting program.")
                time.sleep(1)
                os.kill(os.getpid(), signal.SIGTERM)
        ...
        for mod in options['modalities']:
            ...
            if mod == 'T1':
                ...
                try:
                    print("> PRE:", scan, "skull_stripping the " + CURL + mod + CEND +  " modality")
                    if os_host == 'Windows':
                        print("skull_stripping the " + CURL + mod + CEND +  "modality [",os_host, " system]")
                        subprocess.check_output([options['robex_path'],
                                             t1_im,
                                             t1_st_im])
                    elif os_host == 'Linux':
                    ...
                    # print(p.communicate())
                        ...
                        print("> ERROR:", scan, "registering masks (", mod, ") quiting program.")
                        time.sleep(1)
                        os.kill(os.getpid(), signal.SIGTERM)
    ...
    if options['reg_space'] == 'T1toFlair':
        ...
        try:
            ...
            print("> PRE:", scan, "skull_stripping the " + CURL + "Flair" + CEND +  " modality")
            if os_host == 'Windows':
                print("skull_stripping the" + CURL + "Flair" + CEND +  " modality [",os_host, " system]")
                subprocess.check_output([options['robex_path'],
                                             t1_im,
                                             t1_st_im])
            elif os_host == 'Linux':
                ...
                print("> ERROR:", scan, "registering masks(Flair), quiting program.")
                time.sleep(1)
                os.kill(os.getpid(), signal.SIGTERM)
...
def preprocess_scan(current_folder, options):
    ...
    image = load_nii(os.path.join(options['tmp_folder'], 'FLAIR_brain.nii.gz'))
    image_norm = zscore_normalize(image, mask=None)
    image_norm.to_filename(os.path.join(options['tmp_folder'], 'normalized_FLAIR_X.nii.gz')) 

    image = load_nii(os.path.join(options['tmp_folder'], 'MOD3_brain.nii.gz'))
    image_norm = zscore_normalize(image, mask=None)
    image_norm.to_filename(os.path.join(options['tmp_folder'], 'normalized_MOD3_X.nii.gz'))

    image = load_nii(os.path.join(options['tmp_folder'], 'T1_brain.nii.gz'))
    image_norm = zscore_normalize(image, mask=None)
    image_norm.to_filename(os.path.join(options['tmp_folder'], 'normalized_T1_X.nii.gz'))        
